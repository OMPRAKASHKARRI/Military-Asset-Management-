...
export default function Purchases() {
  ...
  const canCreate = ["ADMIN", "LOGISTICS_OFFICER", "BASE_COMMANDER"].includes(user?.role);
  ...
  async function handleSubmit(e) {
    ...
    if (!form.baseId || !form.equipmentTypeId || !form.date) {
    ...
    setSubmitting(true);
    try {
      await api.post("/purchases", { ...form, quantity: qty });
      ...
      <div className="bg-navy-850 border border-navy-700 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-navy-700">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-200">Purchase History</h3>
          </div>
          {canCreate && (
        ...
        {loading ? (
          ...
          <EmptyState label="No purchases recorded yet" sub="Create one to get started" />
        ) : (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-navy-700/60 hover:bg-navy-800/50 transition-colors">
                    <td className="px-5 py-3 text-slate-300">{new Date(r.date).toLocaleDateString()}</td>
                    ...
                    <td className="px-5 py-3">
                      <Badge variant={r.equipmentType?.category}>{r.equipmentType?.category}</Badge>
                    </td>
                    <td className="px-5 py-3 text-right tabular-nums text-emerald-400 font-medium">+{r.quantity}</td>
                    <td className="px-5 py-3 text-slate-400">{r.createdBy?.username}</td>
                  </tr>
                ))}
              </tbody>
            </table>
