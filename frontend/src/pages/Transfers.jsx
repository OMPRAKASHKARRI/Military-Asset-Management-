...
export default function Transfers() {
  ...
  function openModal() {
    setForm({
      sourceBaseId: isCommander ? user.baseId : "",
      destinationBaseId: "",
      equipmentTypeId: "",
      quantity: "",
    });
  ...
  async function handleSubmit(e) {
    ...
    if (!form.sourceBaseId || !form.destinationBaseId || !form.equipmentTypeId) {
    ...
    setSubmitting(true);
    try {
      await api.post("/transfers", { ...form, quantity: qty });
    ...
    <div className="space-y-5">
      ...
      <div className="bg-navy-850 border border-navy-700 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-navy-700">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-200">Transfer History</h3>
          </div>
        ...
        {loading ? (
          ...
          <EmptyState label="No transfers recorded yet" />
        ) : (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-navy-700/60 hover:bg-navy-800/50 transition-colors">
                    <td className="px-5 py-3 text-slate-300">{new Date(r.timestamp).toLocaleDateString()}</td>
                    <td className="px-5 py-3 text-slate-300">{r.sourceBase?.name}</td>
                    <td className="px-5 py-3 text-slate-300">{r.destinationBase?.name}</td>
                    <td className="px-5 py-3 text-slate-200 font-medium">{r.equipmentType?.name}</td>
                    <td className="px-5 py-3 text-right tabular-nums text-slate-200 font-medium">{r.quantity}</td>
                    <td className="px-5 py-3">
                      <Badge variant={r.status}>{r.status}</Badge>
                    </td>
                    <td className="px-5 py-3 text-slate-400">{r.initiatedBy?.username}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FormField label="Destination Base">
            <select
            ...
            >
              ...
              {bases
                .filter((b) => String(b.id) !== String(form.sourceBaseId))
                .map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.name}
                  </option>
