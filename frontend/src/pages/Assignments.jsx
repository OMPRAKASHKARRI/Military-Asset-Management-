...
export default function Assignments() {
  ...
  const [form, setForm] = useState({ baseId: "", equipmentTypeId: "", personnelName: "", quantity: "" });
  ...
  async function handleSubmit(e) {
    ...
    if (!form.baseId || !form.equipmentTypeId || !form.personnelName.trim()) {
    ...
    setSubmitting(true);
    try {
      await api.post("/assignments", { ...form, quantity: qty });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-navy-850 border border-navy-700 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-navy-700">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-200">Assignment History</h3>
          </div>
        ...
        {loading ? (
          ...
          <EmptyState label="No assignments recorded yet" />
        ) : (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-navy-700/60 hover:bg-navy-800/50 transition-colors">
                    <td className="px-5 py-3 text-slate-300">{new Date(r.assignedAt).toLocaleDateString()}</td>
                    ...
                    <td className="px-5 py-3 text-slate-300">{r.personnelName}</td>
                    <td className="px-5 py-3 text-right tabular-nums text-accent-light font-medium">{r.quantity}</td>
                    <td className="px-5 py-3 text-slate-400">{r.assignedBy?.username}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal
        ...
        title="New Equipment Assignment"
        footer={
          <>
